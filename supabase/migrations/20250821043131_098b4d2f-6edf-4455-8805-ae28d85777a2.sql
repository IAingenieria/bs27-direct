...
CREATE TABLE public.vehiculos (
    ...
    dias_estadia INTEGER GENERATED ALWAYS AS (
        CASE 
            WHEN fecha_entrega IS NULL THEN 
                EXTRACT(days FROM now() - fecha_recibo)::INTEGER
            ELSE 
                EXTRACT(days FROM fecha_entrega - fecha_recibo)::INTEGER
        END
    ) STORED,
    costo_estadia DECIMAL(10,2) GENERATED ALWAYS AS (
        CASE 
            WHEN (CASE 
                WHEN fecha_entrega IS NULL THEN 
                    EXTRACT(days FROM now() - fecha_recibo)::INTEGER
...
CREATE TABLE public.pagos (
    ...
    dias_vencido INTEGER GENERATED ALWAYS AS (
        CASE 
            ...
            WHEN now() > fecha_vencimiento THEN 
                EXTRACT(days FROM now() - fecha_vencimiento)::INTEGER
            ELSE 0
        END
    ) STORED,
...
-- Create RLS Policies (Admin access for now - authentication will be added later)
CREATE POLICY "Enable all access for authenticated users" ON public.clientes FOR ALL USING (true);
CREATE POLICY "Enable all access for authenticated users" ON public.cotizaciones FOR ALL USING (true);
CREATE POLICY "Enable all access for authenticated users" ON public.vehiculos FOR ALL USING (true);
CREATE POLICY "Enable all access for authenticated users" ON public.pagos FOR ALL USING (true);
CREATE POLICY "Enable all access for authenticated users" ON public.notificaciones FOR ALL USING (true);
CREATE POLICY "Enable all access for authenticated users" ON public.contactos FOR ALL USING (true);
